...
new trackdb()
	...
	->import( db_type ); ## create data or database from another format
	V->export( db_type ); ## convert database to another format
	V->crack(); ## crack the database for password
	...
	V->get( data_path ); ## retrieve data from database
		V ->filter( filter ); ## create a filter
